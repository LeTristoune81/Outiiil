...
(function(){
  ...
  const OUTIIL_BOITES = [
    ...
    `${CDN}/js/boite/Dock.js`,     // ton Dock (ton dépôt doit être à jour)
    `${CDN}/js/boite/Map.js`,
  ...
  // Charge tout Outiiil dans l'ordre — (à appeler après que $ soit dispo)
...
(async function OUTIIL_BOOT(){
  ...
  // 2) récupérer jQuery de la page via unsafeWindow et l'exposer
  await new Promise(resolve => {
    const start = Date.now();
    (function waitJQ(){
      try {
        const jq = (unsafeWindow && (unsafeWindow.jQuery || unsafeWindow.$)) || window.jQuery || window.$;
        if (jq) {
          window.jQuery = jq;
          window.$ = jq;
          return resolve();
      ...
      if (Date.now() - start > 8000) return resolve(); // ne pas bloquer indéfiniment
      setTimeout(waitJQ, 50);
    })();
  });

  // 3) charger dynamiquement libs/classes/boîtes/pages
  await window.outiilLoadAll();
  console.log("[Outiil] jQuery lié:", typeof $, "— modules prêts, on initialise…");

  // 4) si l'utilisateur est identifié
  if ($(".boite_connexion_titre:first").text() != "Connexion") {
    ...
    Promise.all([monProfil.getConstruction(), monProfil.getLaboratoire(), monProfil.getProfilCourant()]).then((values) => {
      ...
      // Dock
      const boite = new Dock();
      boite.afficher();

      // Garde-fou : retirer Traceur/Carte si une vieille version les remet
      $("#o_toolbarOutiiil")
        .find('.o_toolbarItem[title="Traceur"], .o_toolbarItem[title="Carte"], #o_itemTraceur, #o_itemMap')
        .closest(".o_toolbarItem").remove();

      // Boîtes annexes
      const boiteComptePlus = new BoiteComptePlus(); boiteComptePlus.afficher();
      const boiteRadar      = new BoiteRadar();      boiteRadar.afficher();

      // Traceur auto si clé
      if (monProfil.parametre["cleTraceur"] && monProfil.parametre["cleTraceur"].valeur) {
        const t1 = new TraceurJoueur(
      ...
      switch (true) {
        ...
        case (uri == "/construction.php"):
          page = new PageConstruction(boiteComptePlus); page.executer(); break;
        case (uri == "/laboratoire.php"):
          page = new PageLaboratoire(boiteComptePlus); page.executer(); break;
        case (uri == "/Ressources.php"):
          page = new PageRessource(boiteComptePlus);   page.executer(); break;
        case (uri == "/Armee.php"):
          page = new PageArmee(boiteComptePlus);       page.executer(); break;
        case (uri == "/commerce.php"):
          page = new PageCommerce(boiteComptePlus);    page.executer(); break;
        case (uri == "/messagerie.php"):
          page = new PageMessagerie();                 page.executer(); break;
        case (uri == "/alliance.php" && location.search == ""):
        case (uri == "/chat.php"):
          page = new PageChat();                       page.executer(); break;
        case (location.href.indexOf("/alliance.php?forum_menu") > 0):
          page = new PageForum();                      page.executer(); break;
        case (location.href.indexOf("/alliance.php?Membres") > 0):
          page = new PageAlliance();                   page.executer(); break;
        case (location.href.indexOf("/Membre.php?Pseudo") > 0):
        case (uri == "/Membre.php"):
          page = new PageProfil(boiteRadar);           page.executer(); break;
        case (uri == "/classementAlliance.php" && Utils.extractUrlParams()["alliance"]):
        ...
        case (uri == "/ennemie.php" && location.search == ""):
          ...
          $("#tabEnnemie tr:gt(0)").each((i, elt) => {
            const distance = parseInt($(elt).find("td:eq(5)").text(), 10);
