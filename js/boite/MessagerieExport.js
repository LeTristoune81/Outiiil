...
(function () {
  ...
  // --- mini utils sans dépendances ---
  const $  = (s, r=document) => r.querySelector(s);
  const $$ = (s, r=document) => Array.from(r.querySelectorAll(s));
  const sleep = ms => new Promise(r => setTimeout(r, ms));
  const addStyle = (css) => { const st=document.createElement('style'); st.textContent = css; document.head.appendChild(st); };
  const copyText = async (txt) => {
    try { if (navigator.clipboard?.writeText) return await navigator.clipboard.writeText(txt); } catch(_) {}
  ...
  class BoiteMessagerieExport {
    constructor() { this._inj = new WeakSet(); }
    css() {
      addStyle(`
    ...
    makeCopyBtn(ta, label) {
      const b = document.createElement('button'); b.className='zz-mini'; b.textContent=label;
      b.onclick = () => copyText(ta.value);
      return b;
    }
    inject(table) {
      if (this._inj.has(table)) return; this._inj.add(table);

      const rBtn = table.insertRow(-1), cBtn = rBtn.insertCell(0);
      cBtn.colSpan = table.rows[0]?.cells.length || 2; cBtn.style.textAlign='center';
      ...
      actions.append(
        this.makeCopyBtn(taRaw, 'Copier Texte'),
        this.makeCopyBtn(taFZ, 'Copier BBCode Fzzz'),
        this.makeCopyBtn(taMD, 'Copier Markdown')
      );
    ...
    scan() {
      $$('tr.contenu_conversation td > table').forEach(t => this.inject(t));
    }
    boot() {
      if (!/\/messagerie\.php$/i.test(location.pathname)) return;
      this.css();
      this.scan();
      this._mo?.disconnect();
      this._mo = new MutationObserver(() => this.scan());
      this._mo.observe(document.body, { childList:true, subtree:true });
    }
  }

  // Expose façon “boite”
  window.BoiteMessagerieExport = BoiteMessagerieExport;
})();
