// ===== Outiiil — Module Messagerie Export (Objet / Participants / Horodatage / Départs-Rejoins / Text, BBCode Fzzz, Markdown) =====
(function () {
  'use strict';

  // -- Petits utilitaires compatibles TM/GM/Vanilla
  ...
  function makeCopyBtn(ta, label) {
    ...
    b.onclick = () => copyToClipboard(ta.value);
    return b;
  }
  ...
  function bootCore() {
    if (!/\\/messagerie\\.php(?:$|\\?)/i.test(location.pathname + location.search)) return;
  ...
  // -- Intégration Outiiil (toolbar) + fallback autonome
  function addToolbarButtonIfOutiiil() {
    try {
      const tb = document.querySelector('#o_toolbarOutiiil');
      if (!tb) return false;

      const item = document.createElement('div');
      item.className = 'o_toolbarItem';
      item.style.cursor = 'pointer';
      item.innerHTML = `<span>Exporter messagerie</span>`;
      item.addEventListener('click', () => {
        if (!/\\/messagerie\\.php/i.test(location.href)) {
          location.href = location.origin + '/messagerie.php';
          return;
        }
        $$('tr.contenu_conversation td > table').forEach(inject);
